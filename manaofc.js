...
//======================================
const CONFIG_URL = "https://raw.githubusercontent.com/buddika-iresh17/hi/refs/heads/main/config.js"; // <-- ඔබේ GitHub config link එක දාන්න

async function loadRemoteConfig() {
  try {
    const configPath = path.join(__dirname, "config.js");
    console.log("🌐 Downloading config from:", CONFIG_URL);

    // Download config.js
    const res = await axios.get(CONFIG_URL, { responseType: "text" });
    fs.writeFileSync(configPath, res.data, "utf8");

    console.log("✅ Config saved successfully:", configPath);

    // Reload config.js dynamically
...
let config; // Global config variable
...
if (!fs.existsSync(sessionFile)) {
  if (!settings.SESSION_ID) {
    console.log('🌀 ᴍᴀɴɪꜱʜᴀ-ᴍᴅ 💕 Please add your session id in settings.SESSION_ID! 😥...');
  } else {
    const sessdata = settings.SESSION_ID.replace("manaofc~", "");
...
async function connectToWA() {
 if (!config) config = await loadRemoteConfig(); // load config before connection
...
║ 📜 Bot Description:  
╠════════════➢
║ ${BOT} is a powerful, multipurpose WhatsApp bot
...
${CREATER}`;
        ...
        const handler = async (update) => {
            ...
            if (repliedTo !== sentMessage.key.id) return; // ensure reply is to our message

            try {
                if (replyText === "1") {
                    // Send as video
                    await conn.sendMessage(from, { video: { url: info.dlink }, caption: `${CREATER}`}, { quoted: m });
                } else if (replyText === "2") {
                    // Send as document
                    await conn.sendMessage(from, {
                        document: { url: info.dlink, mimetype: 'video/mp4', fileName: `${info.title}.mp4` },
                        caption: `${CREATER}`
                    }, { quoted: m });
                } else {
                    await conn.sendMessage(from, { text: "❌ Invalid choice. Reply 1 or 2." }, { quoted: m });
                }
            } catch (err) {
                console.error(err);
                await reply("❌ Error sending the video.");
            } finally {
                // Stop listening after the choice
...
${CREATER}`;
    ...
    const handler = async (update) => {
      ...
      if (repliedTo !== sentMessage.key.id) return; // only handle replies to our message
      ...
      try {
        ...
        conn.ev.off("messages.upsert", handler); // stop listening after one reply
      }
    };
