...
const {
  default: makeWASocket,
    ...
    DisconnectReason,
    jidNormalizedUser,
    getContentType: baileysGetContentType,
   // getContentType,
    proto,
...
function getContentType(message) {
    return baileysGetContentType(message);
}
...
const sms = async (conn, m, store) => {
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            if (!quoted.viewOnceMessageV2) {
                ...
                // create fakeObj using generateWAMessageFromContent
                m.quoted.fakeObj = await generateWAMessageFromContent(
                    m.quoted.chat,
                    { ...quoted },
                    {
                ...
                m.forwardMessage = (jid, forceForward = true, options = {}) => conn.copyNForward(jid, m.quoted.fakeObj, forceForward, { contextInfo: { isForwarded: false } }, options);
    ...
    // copy message
    m.copy = async () => await generateWAMessageFromContent(m.chat, { ...m.msg }, { messageId: m.id });
    ...
    m.reply = async (content, opt = { packname: "Secktor", author: "SamPandey001" }, type = "text") => {
        switch (type.toLowerCase()) {
            ...
            case "image":
                ...
                if (isUrl(content)) return await conn.sendMessage(m.chat, { image: { url: content }, ...opt }, { quoted: m });
                break;
            case "video":
                if (Buffer.isBuffer(content)) return await conn.sendMessage(m.chat, { video: content, ...opt }, { quoted: m });
                if (isUrl(content)) return await conn.sendMessage(m.chat, { video: { url: content }, ...opt }, { quoted: m });
                break;
            case "audio":
                if (Buffer.isBuffer(content)) return await conn.sendMessage(m.chat, { audio: content, ...opt }, { quoted: m });
                if (isUrl(content)) return await conn.sendMessage(m.chat, { audio: { url: content }, ...opt }, { quoted: m });
                break;
            case "sticker":
                const { writeExifWebp } = require('./functions'); // your sticker helper
